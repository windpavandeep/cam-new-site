        ...
        <form method="POST" action="{{ route('dashboard.jobs.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="12" required maxlength="65000"
                    class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 focus:border-sky-600 focus:outline-none focus:ring-1 focus:ring-sky-500">{{ old('description') }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_published" id="is_published" value="1" class="h-4 w-4 rounded border-slate-300 text-sky-700 focus:ring-sky-500" {{ (string) old('is_published', '1') === '1' ? 'checked' : '' }}>
